...
router.get('/pull', auth.routerActivityByToken, async function (req, res) {
    ...
    if (!keys) {
        ...
        const danmaku = await Promise.all(
            keys.map(
                async (key) => JSON.parse(await getAsync(key))
            )
        );
...
router.get('/url', async function (req, res) {
    ...
    let url = {};
    const info = { host: config.host + config.rootPath + "/danmaku", query: { acitivity: params.activity, tokenName: params.name, token: params.token} };
    url.danmaQ = "danmaQ://" + Buffer.from(JSON.stringify(info)).toString("base64");
                        ...
                        audit.filters(danmaku, activity, function (autherr) {
                            if (autherr) {
                                ...
                                if (danmaku.status === "publish") {
                                    ...
                                    redis.setex(`acitvity:${activity.id}:danmaku:${danmaku.id}`, config.danmaku.expire, JSON.stringify(danmaku));
                                }
                                else if (danmaku.status === "audit")
                                    redis.sadd(`acitvity:${activity.id}:audit`, `${JSON.stringify(danmaku)}`);
        ...
        if (perms.has("audit")) {
            socket.on("get audit list", function (data) {
                ...
                if (count > 20 || count <= 0) {
                    ...
                    redis.spop(`acitvity:${activity.id}:audit`, count, function (err, res) {
                        if (err || !res) socket.emit("message", "nothing to audit");
                        else {
                            socket.emit("get audit list", res.map(i => JSON.parse(i)));
                        }
                    });
                }
            });
            ...
            socket.on("set audit status", function (data) {
                if (!data.id || !data.status) { socket.emit("message", "missing parameter"); }
                            ...
                            redis.expire(lockKey, 10, function () {
                                Danmaku.getDanmaku(data.id, function (err, danmaku) {
                                    if (err || danmaku.activity != activity.id ) {
                                        ...
                                        danmaku.updateStatus(data.status, function (err) {
                                            if (err) {
                                                socket.emit("message", "unknown error");
                                            } else {
                                                if (danmaku.status === "publish") {
