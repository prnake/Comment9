          ...
          <el-col :xs="{ span: 24 }" :sm="{ span: 22 }" :md="{ span: 20 }">
            ...
            <div v-else>
              ...
              <div
                class="container"
                v-if="
                  activityEditInfo.senders &&
                  activityEditInfo.senders.includes('danmaku')
                "
              >
                <h1 class="height-5">{{ $t("Danmaku Address") }}</h1>
                <p class="font-20">
                  {{
                    $t(
                      "Urls are generated by the tokens we have above, so please ensure that you are using url with the right token."
                    )
                  }}
                </p>
                <p class="height-5">
                ...
                <p class="height-5">
                  {{ $t("Danmaku Web Sender") }}
                  <el-popover
                    v-for="perm in ['pull', 'push']"
                    v-bind:key="perm"
                    trigger="hover"
                    placement="top"
                    width="200"
                ...
                <el-input
                  class="font-16 pad-right-130"
                  v-for="url in danmakuWebSenderUrl"
                  v-bind:key="url"
                  :value="url.url"
                  :disabled="true"
                >
